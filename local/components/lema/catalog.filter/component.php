    ...
    while ($arProp = $rsProp->Fetch()) {
        if (in_array($arProp["CODE"], $arParams["PROPERTY_CODE"]) && $arProp["PROPERTY_TYPE"] != "F") {
            ...
            if ($arProp["PROPERTY_TYPE"] == "L") {
                ...
                $arrEnum = array();
                /** Required for property "COLOR" **/
                if($arProp['CODE'] == "COLOR") {
                    $property_enums = CIBlockPropertyEnum::GetList(
                        Array("SORT"=>"ASC","xml_id" => "ASC"),
                        Array("IBLOCK_ID" => $arParams['IBLOCK_ID'], "CODE" => $arProp['CODE'])
                    );
                    while ($arEnum = $property_enums->Fetch()) {
                        $arrEnum[$arEnum["ID"]] = $arEnum['XML_ID'];
                    }
                }else{
...
foreach ($arResult["arrProp"] as $prop_id => $arProp) {
    ...
    if ($arProp['CODE'] == "TYPE_FUR" || $arProp['CODE'] == "COLOR") {
        $arProp['LIST_TYPE'] = "C";
        $arProp['MULTIPLE'] = "Y";
    }
        ...
        case "L":
            ...
            if ('C' == $arProp['LIST_TYPE']) {
                ...
                if ('Y' == $arProp['MULTIPLE']) {
                    ...
                    if ($arProp['CODE'] == "TYPE_FUR") {
                        $arListRadio[] = ' <div class="filter-form__wrap">
                                                <div class="filter-form__group">
                                                    <label class="filter-form__label">
                                                        <input class="filter-form__checkbox checked-all-js"
                                                         type="checkbox"
                                                          name="' . $name . '[]"
                                                          ' . (in_array(all, $arListValue) ? ' checked' : '') . '
                                                          value="all">
                                                        <div class="filter-form__custom"></div>
                                                        <div class="filter-form__text">
                                                            ' . GetMessage("CC_BCF_ALL") . '
                                                        </div>
                                                    </label>
                                                </div>';
                        foreach ($arProp["VALUE_LIST"] as $key => $val) {
                            $arListRadio[] = '<div class="filter-form__group">
                                                    <label class="filter-form__label">
                                                        <input class="filter-form__checkbox"
                                                         type="checkbox"
                                                          name="' . $name . '[]"
                                                          value="' . htmlspecialcharsbx($key) . '"
                                                          ' . (in_array($key, $arListValue) ? ' checked' : '') . '>
                                                        <div class="filter-form__custom"></div>
                                                        <div class="filter-form__text">
                    ...
                    } elseif ($arProp['CODE'] == "COLOR") {
                        $arListRadio[] = '<div class="filter-form__color">';
                        foreach ($arProp["VALUE_LIST"] as $key => $val) {
                            $arListRadio[] = '<label class="filter-form__color-label">
                                                    <input class="filter-form__radio"
                                                     type="checkbox"
                                                      name="' . $name . '[]"
                                                      value="' . htmlspecialcharsbx($key) . '"
                                                      ' . (in_array($key, $arListValue) ? ' checked' : '') . '>
                                                    <div class="filter-form__color-custom">
                                                        <div class="filter-form__color-btn color-'.$val.'"></div>
                                                    </div>
                                                </label>';
                        }
                    $arListRadio[]= '</div>';
